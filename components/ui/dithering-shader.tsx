...
export function DitheringShader({
  ...
  const autoSize = fullscreen || (width === undefined && height === undefined)
  const [size, setSize] = useState<{ w: number; h: number }>({
    w: width ?? 800,
    h: height ?? 800,
  })

  // Observe container size when in auto-size mode
  useEffect(() => {
    if (!autoSize) return
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (locations.u_resolution) context.uniform2f(locations.u_resolution, size.w, size.h)
  ...
  }, [size.w, size.h, colorBack, colorFront, shape, type, pxSize, speed])

  const containerStyle: React.CSSProperties = autoSize
    ? { position: "relative", width: "100%", height: "100%", ...style }
    : { position: "relative", width: size.w, height: size.h, ...style }
